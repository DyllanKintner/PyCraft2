...
def open_survival_inventory():
    global inventory_opened, survival_inventory_panel, cs1, cs2, cs3, cs4, result_slot, inventorycraftingslots
...
def take_result_item(s):
    global inventory_crafting_grid
    if holding_block:
        return
    else:
        for i in inventorycraftingslots:
...
def update():
    global fov_slider, issprinting, iscrouching, paused, last_y_position, fall_start_y, health, is_falling, light_to_dark, currently_breaking_block, block_break_start_time, mouse_held, overlay_entity, mining_animation_running, coordslabel, tiptext, title_screen_open, slot1amount, inventory_crafting_grid
    if paused:
