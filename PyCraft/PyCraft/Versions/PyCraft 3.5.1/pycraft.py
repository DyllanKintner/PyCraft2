...
inventory_recipes = {
    ('OakPlanksVoxel', 'OakPlanksVoxel', 'OakPlanksVoxel', 'OakPlanksVoxel'): (CraftingTableVoxel, 1),
    ('OakLogVoxel', None, None, None): (OakPlanksVoxel, 4),
...
def open_survival_inventory():
    global inventory_opened, survival_inventory_panel, cs1, cs2, cs3, cs4, result_slot, inventorycraftingslots, cs1amount, cs2amount, cs3amount, cs4amount, rsamount, right_click_function_buttons
    ...
    inventorycraftingslots = [cs1,cs2,cs3,cs4]
    for i in inventorycraftingslots:
        right_click_function_buttons.append(i)
    result_slot = Button( 
                    color=color.hsv(0, 0, 0.9),
...
def add_item_to_crafting_slot(cs):
    global inventory_crafting_grid, result_slot
    if holding_block:
        equip_block(cs)
    else:
        swap_block(cs)
    ...
    if not creative and inventory_opened:
        for i in inventorycraftingslots:
            inventory_crafting_grid[i.gridslot] = i.equipped.__name__ if i.equipped else None
    print(inventory_crafting_grid)
    crafting_tuple = tuple(inventory_crafting_grid)
    print(crafting_tuple)
    if crafting_tuple in inventory_recipes:
        print('recipe found!')
        result_slot.texture = inventory_recipes[crafting_tuple][0].block_icon
        result_slot.visible = True
        result_slot.equipped = inventory_recipes[crafting_tuple][0]
        result_slot.hand_color = inventory_recipes[crafting_tuple][0].block_color
        result_slot.amount = inventory_recipes[crafting_tuple][1]
    else:
...
def take_result_item(s):
    global inventory_crafting_grid, block_drag, block_drag_amount
...
def right_click_equip(slot):
    global block_drag, block_drag_amount, holding_block, inventory_crafting_grid
...
def update():
    global fov_slider, issprinting, iscrouching, paused, last_y_position, fall_start_y, health, is_falling, light_to_dark, currently_breaking_block, block_break_start_time, mouse_held, overlay_entity, mining_animation_running, coordslabel, tiptext, title_screen_open, slot1amount, inventory_crafting_grid, right_click_held
    if paused:
